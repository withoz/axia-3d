...
async function main() {
  ...
  // Export panel manager for global access
  (window as any).__axia_panelManager = panelManager;

  // Setup file operation globals
  (window as any).__axia_open = async () => {
    const success = await fileManager.openProject();
    if (success) {
  ...
  (window as any).__axia_fileManager = fileManager;
  ...
  // Export globals for debugging & tools
  (window as any).__axia_units = units;
  (window as any).__axia_bridge = bridge;
  (window as any).__axia_viewport = viewport;
  (window as any).__axia_importer = fileImporter;
  // toolManager도 디버깅용으로 노출
  // (아래 초기화 후 설정)
  ...
  // 4. Initialize tool manager (connects bridge ↔ viewport ↔ units)
  const toolManager = new ToolManager(viewport, bridge, units);
  (window as any).__axia_toolManager = toolManager;

  // 4a. Initialize command input (CAD-style commands)
  const commandInput = new CommandInput();
  (window as any).__axia_commandInput = commandInput;
